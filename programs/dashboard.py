...
import json
import questionary
from programs import chemistry as chem
...
def main(user_info):
    while True:
        ...
        print("\n")
        cyan(
            f"⭐  {random.choice(prompts.get('greetings', ['Hello']))}, {user_info.get('forename', 'User')}! ⭐"
        )
        print()

        menu(
            "What do you want to do", ["Learn", "Revise", "Quiz", "Flashcards", "Quit"]
        )
        ...
        if choice == "Quit":
            ...
            if not isinstance(xp, int) or not (0 <= xp <= 100):
                raise ValueError(
                    "user_info.json 'xp' must be an integer between 0 and 100"
                )
